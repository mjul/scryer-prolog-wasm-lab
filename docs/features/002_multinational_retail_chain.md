...
- in app/multinationals/index.tsx add basic screen  using shadcn components. Add these as necessary.
    ...
    - show a Card with header "Companies" with a list of the companies. This is the master view in the card. The detail view for the selected company is also in the card, when selected: Show the name of the company, its Currency (if defined) and a Tree with its subsidiaries and retail locations. 
